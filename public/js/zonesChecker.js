...
var mouvementPagesZones=[]
    for (var pagenum in scoreParts.allPagesZones.pages) {
      ...
      if (scoreParts.allPagesZones.numberOfVoices) {
        ...
        if (modulo != 0) self.addMessage(pagenum, '', 'bad number of zones' + page.length);
      }

      page.forEach(function (zone, zoneIndex) {
        if (page.x < 0) self.addMessage(pagenum, zoneIndex, 'bad x' + zone.x);
        if (page.y < 0) self.addMessage(pagenum, zoneIndex, 'bad y' + zone.y);
      });
    }
