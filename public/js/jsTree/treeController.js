...
var treeController = (function () {
  ...
  self.onSelect = function (jstreeNode) {
    if (jstreeNode.id) {
      ...
      if (jstreeNode.data) obj.data = jstreeNode.data;
      callback('infos', obj);
    }
  };

  self.onRename = function (jstreeNode) {
    // on cree ou renomme les noeuds lors de l'évenement rename (after)
    var parent = jstreeNode.parent;

    if (jstreeNode.original) {
  ...
  self.load = function (
    ...
    plugins.push('sort');
    plugins.push('types');
    if (checkedNodes) plugins.push('checkbox');
    if (callback) plugins.push('contextmenu');
    ...
    var jsTree = $('#' + jsTreeDivId)
      .on('loaded.jstree', function (e, data) {
        ...
        } else if (unfoldAll) $('#' + jsTreeDivId).jstree('open_all');
        else $('#' + jsTreeDivId).jstree('open_node', '#' + rootId);
      })
      ...
      .on('changed.jstree', function (e, data) {
        ...
        if (data && data.node && data.node.id) {
          if (data.selected.indexOf(data.node.id) > -1) {
            ...
            if (selectedNodes[data.node.id]) delete selectedNodes[data.node.id];
          }
        }
      })
