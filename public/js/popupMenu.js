var popupMenu = (function () {
  ...
  self.showGraphPopupMenu = function (zoneGroup, windowPoint, paperPoint) {
    ...
    if (true) {
      ...
      html +=
        '    <span class="popupMenuItem" onclick="popupMenu.setMeasure()"> Ajouter Numero mesure</span>';
      html += '    <span class="popupMenuItem" onclick="popupMenu.addText()"> Ajouter texte</span>';
    } else {
      html =
        '    <span class="popupMenuItem" onclick="GraphController.graphActions.removeNodeFromGraph();"> Remove Node</span>';
    }
  ...
  self.setMeasure = function () {
    ...
    if (number) {
      self.currentZone.data.measure = {
        x: self.currentPaperPoint.x,
        y: self.currentZone.bounds.y,
        number: number,
      };
      Paper.drawMeasure(
        self.currentZoneGroup,
        self.currentPaperPoint.x,
        self.currentZone.bounds.y,
        number
      );
      PopupMenuWidget.hidePopup('popupMenuWidgetDiv');
    }
