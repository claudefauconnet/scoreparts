  ...
  const COLOR_PALETTE = ['#c9534b','#d99441','#4a7a8c','#6b6396','#5e8c61','#a86b9b','#3a342b','#b08a3e'];
  ...
  function renderInstruments() {
    ...
    INSTRUMENTS.forEach((ins, idx) => {
      ...
      row.querySelector('[data-toggle]').addEventListener('click', (e) => {
        e.stopPropagation(); ins.on = !ins.on; renderInstruments(); renderZones();
      });
      row.querySelector('[data-del]').addEventListener('click', (e) => {
        e.stopPropagation();
        document.querySelectorAll('.instr-pop').forEach(p => p.remove());
        document.querySelectorAll('.instr.has-open-pop').forEach(r => r.classList.remove('has-open-pop'));
      ...
      row.addEventListener('click', () => { activeInstr = ins.id; renderInstruments(); });
  ...
  function makeZone(instr, sys, label) {
    return { id: 'z' + Math.random().toString(36).slice(2,7), instr, top: sys * SYS_H + 1, height: SYS_H - 4, x: 4, w: 94, label };
  }
  let ZONES_LEFT = [
  ...
  function renderZones() {
    [['zones-left', ZONES_LEFT], ['zones-right', ZONES_RIGHT]].forEach(([containerId, zones]) => {
  ...
  function attachZoneEvents(div, z, zones, containerId) {
    ...
    div.querySelector('[data-act="switch"]').addEventListener('mousedown', e => e.stopPropagation());
    ...
    div.querySelector('[data-act="delete"]').addEventListener('mousedown', e => e.stopPropagation());
  ...
  function renderMvt() {
    ...
    document.getElementById('mvt-number').innerHTML = 'Mouvement <span class="roman">' + (ROMAN[order] || (order+1)) + '</span>';
    ...
    MOVEMENTS.forEach((mv, i) => {
      ...
      `;
      item.addEventListener('click', () => { activeMvt = mv.id; renderMvt(); closeMvtPop(); });
  ...
  function openMvtPop() { document.getElementById('mvt-pop').classList.add('open'); }
  function closeMvtPop() { document.getElementById('mvt-pop').classList.remove('open'); }
  ...
  dlCaret.addEventListener('click', (e) => {
    ...
    sub.textContent = n === 0
      ...
      : (n === 1 ? '1 PDF (instrument actif)' : `${n} PDF (un par instrument actif)`);
  });
  document.addEventListener('click', (e) => {
    ...
    pageEl.addEventListener('mousedown', (e) => {
      ...
      function up(ev) {
        ...
        document.querySelectorAll('.zone').forEach(zEl => {
          ...
          const inter = !(r.right < mRect.left || r.left > mRect.right || r.bottom < mRect.top || r.top > mRect.bottom);
          if (inter) {
    ...
    bar.querySelector('[data-act="instrument"]').addEventListener('click', (e) => {
      ...
      INSTRUMENTS.forEach(ins => {
        ...
        b.addEventListener('click', (ev) => {
          ...
          selectedEls.forEach(zEl => {
            const info = findZoneObj(zEl);
            if (info) { info.z.instr = ins.id; info.z.label = ins.name; }
          });
          pop.remove();
          renderInstruments();
          renderZones();
  ...
  document.addEventListener('mousedown', (e) => {
    if (e.target.closest('.multi-bar') || e.target.closest('.zone.selected') || e.target.closest('.marquee') || e.target.closest('.zone-pop')) return;
    document.querySelectorAll('.zone.selected').forEach(z => z.classList.remove('selected'));
    document.querySelectorAll('.multi-bar').forEach(b => b.remove());
  });
  ...
  function applyRectH() {
    ...
    document.querySelectorAll('.zone').forEach(z => { z.style.minHeight = h + 'px'; });
  }
  rectInput.addEventListener('input', applyRectH);
  document.getElementById('rect-minus').addEventListener('click', () => { rectInput.value = (parseInt(rectInput.value)||40) - 1; applyRectH(); });
  document.getElementById('rect-plus').addEventListener('click', () => { rectInput.value = (parseInt(rectInput.value)||40) + 1; applyRectH(); });
