...
(function selectorModule() {
  ...
  function nodeMatchesQuery(node, query) {
    const searchableText = [node.name, node.author, node.uploader, ...(Array.isArray(node.tags) ? node.tags : [])]
    ...
    for (const node of nodes) {
      if (node.type === 'folder' && node.children) {
        ...
        if (filteredChildren.length || nodeMatchesQuery(node, query)) {
          filteredNodes.push({ ...node, children: filteredChildren.length ? filteredChildren : node.children });
        }
  ...
  function buildNode(node, depth) {
    ...
    const nodeClass = 'node ' + (isLeaf ? 'leaf' : 'folder') + (node.type === 'score' ? ' score' : '');
    ...
    if ($deleteBtn.length) {
      $deleteBtn.on('click', function (event) { onDeleteClick(node, event); });
    }
    ...
    if (!isLeaf) {
      ...
      $row.on('click', function () { onFolderRowClick($wrap, $row); });
    } else {
      $row.on('click', function () { onScoreRowClick(node, $row); });
    }
  ...
  function renderTree() {
    ...
    visibleNodes.forEach(function (treeNode) { $tree.append(buildNode(treeNode)); });
    if (searchQuery) {
      openAllFolders();
    } else {
      openFirstFolder();
